    ...
    override protected void CheckSettings()
    {
        ...
        if (Application.isEditor || OT.dirtyChecks || dirtyChecks)
        {
            ...
            else
            {
				...
				{
					//if (!baseSize.Equals(Vector2.zero))	{
					//	Debug.Log(string.Format("old size: {4}, {5}, new Image size: {0}, {1}, baseSize: {2}, {3}", image.width, image.height, baseSize.x, baseSize.y, size.x, size.y));
		            //   	size =  new Vector2 (_image_.width/2 * (image.width / baseSize.x) , _image_.height/2 * (image.height / baseSize.y) ) * OT.view.sizeFactor;
					//}
					//else
		            	size = new Vector2(image.width/2, image.height/2) * OT.view.sizeFactor;
